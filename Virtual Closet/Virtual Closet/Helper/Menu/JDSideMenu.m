                    ...
                    animated:(BOOL)animated;
{
    ...
    previousController = self.contentController;
    _contentController = contentController;
...
- (void)setDissmissContentController:(DissmissHandler)block
{
   ...
   UIViewController*  previousCon = self.contentController;
    _contentController = previousController;
   ...
   // //CGFloat offset = JDSideMenuDefaultMenuWidth + (self.view.frame.size.width-JDSideMenuDefaultMenuWidth)/2.0;
    ...
    [UIView animateWithDuration:JDSideMenuDefaultCloseAnimationTime/2.0 animations:^{
       // blockSelf.containerView.transform = CGAffineTransformMakeTranslation(offset, 0);
        ...
        [blockSelf.containerView.layer addAnimation:transition forKey:nil];
    } completion:^(BOOL finished) {
        UIView *vew=self.contentController.view;
        if (vew) {
            // move to container view
            [blockSelf.containerView addSubview:self.contentController.view];
            [blockSelf.contentController didMoveToParentViewController:blockSelf];
            ...
            // remove old controller
            [previousCon willMoveToParentViewController:nil];
            [previousCon removeFromParentViewController];
            [previousCon.view removeFromSuperview];
            //previousController = self.contentController;
        }
        block(NO);
//        if (self.isVisible) {
...
//        }
//        else{
//            [blockSelf showMenuAnimated:YES];
//        }
        //[blockSelf hideMenuAnimated:YES];
