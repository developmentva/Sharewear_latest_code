...
- (id)init
{
    ...
    if (self) {
		...
		self.imageAvatar.image=[UIImage imageNamed:@"defaultimage"];
...
- (void)setupView:(NSDictionary*)params
{
	...
	if ([params[@"avatar"] isKindOfClass:[NSString class]]) {
		[self.imageAvatar loadImageFromURL:[NSURL URLWithString:params[@"avatar"]] placeholderImage:nil cachingKey:nil];
	}
